use blst::{min_pk::*, BLST_ERROR, blst_scalar, blst_scalar_from_uint32};
...
fn bench_pop(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("PoP verify (minimal key)");
...
fn bench_pop_batch(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("PoP batch verify (minimal key)");
	for signers in SIGNERS {
		group.bench_with_input(BenchmarkId::from_parameter(signers), signers, |b, &signers| {
			// Generate signing data for a common message
			let msg = gen_msg(&mut rng);
...
fn bench_verify(c: &mut Criterion) {
	...
	let mut group = c.benchmark_group("Signature verify (minimal key)");
	for signers in SIGNERS {
		group.bench_with_input(BenchmarkId::from_parameter(signers), signers, |b, &signers| {
			// Generate signing data for a common message
			let msg = gen_msg(&mut rng);
